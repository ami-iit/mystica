function stgs = getDefaultSettingsSimDynRel(model,input)
    arguments
        model
        input.startFile = ''
    ...
    stgs.stateDyn.nullSpace.toleranceRankRevealing = [1e-8];
    ...
    stgs.integrator.dxdtOpts.optOpts.name    = 'osqp';
    ...
    stgs.integrator.statusTracker.timeTrackerFile.run       = 0;
    ...
    stgs.visualizer.joint.cone.angleIn   = 45*pi/180;
    ...
    stgs.visualizer.gif.name             = ['dynRel_',model.name,'_',strTime,'.gif'];
    stgs.visualizer.gif.compressionRatio = 2;

    stgs.visualizer.video.save    = 0;
    stgs.visualizer.video.name    = ['dynRel_',model.name,'_',strTime,'.mp4'];
